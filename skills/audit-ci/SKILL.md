...
| **mutation.yml** | present + separate workflow for any repo with a .NET test project; no `continue-on-error`; `break: 0`; measured cadence; ordinary one-project lane runs from the intended unit-test project directory; MTP config has no `test-case-filter`; intentional multi-project lanes use documented `test-projects` from the project-under-test directory; discovered-test count matches the lane and known-tested code produces a non-zero `Killed` count | gap: missing on a .NET repo; drift: run as a `ci.yml` gate; drift: inert MTP filter; drift: unverified discovery/result attribution |

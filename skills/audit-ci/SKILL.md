...
| **mutation.yml** | present + separate workflow for any repo with a .NET test project; no `continue-on-error`, `break: 0`, dispatch + measured cadence; **Stryker step has `working-directory` set to the UNIT test project** | gap: missing on a .NET repo; drift: run as a `ci.yml` gate; **drift: `dotnet stryker` run from the repo root — it then runs EVERY test project referencing the target, whatever `test-projects` says** |
| **`stryker-config.json` scoping** | `test-runner: mtp`; no `test-case-filter` (silently ignored under `mtp` — VSTest-only); no reliance on `test-projects` (does not restrict execution); slow/database-backed tests live in a SEPARATE test project the config does not point at | drift: `test-case-filter` present with `test-runner: mtp` — a filter that has never run; drift: `test-runner: vstest` against an xunit.v3 MTP executable — produces invalid results (0 kills, timeouts scored as kills) |
